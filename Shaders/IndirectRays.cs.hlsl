    ...
    for( uint i = 0; i < pathNum; i++ )
    {
        ...
        for( uint bounceIndex = 1; bounceIndex <= desc.bounceNum && !geometryProps.IsSky(); bounceIndex++ )
        {
...

            {
                ...
                - transforms radiance into non-radiance, which is strictly speaking not allowed to be processed spatially (who want to get a high energy firefly
                  redistributed around surrounding pixels?)
                - not necessarily converges to the right images, because we do assumptions about the future and approximate the tail of the path via a scaling factor
                - this approach breaks denoising, especially REBLUR, which has been designed to work with pure radiance
                */
                float russianRouletteProbability = STL::Color::Luminance( BRDF );
                if( russianRouletteProbability < STL::Rng::Hash::GetFloat( ) )
                    break;

                BRDF /= russianRouletteProbability;
            #endif
